		...
		<div className={cn('relative', showErrors && 'shadow-sm bg-muted')}>
			<div className='absolute top-1 right-2 flex items-center gap-2 divide-neutral-950'>
				...
				{isValid ? (
					...
					<span className='text-xs font-medium font-mono text-destructive'>
						Invalid
					</span>
				)}
				<Button
					size='icon'
					variant='ghost'
					onClick={() => handleCopy(JSON.stringify(form.watch(), null, 2))}>
					{hasCopied ? (
			...
			<pre
				className={cn(
					'py-2 overflow-auto border px-3 bg-muted shadow-sm text-foreground font-mono rounded-md',
					showErrors && errorss.length > 0 && 'rounded-b-none shadow-none',
				)}>
