...
interface DataTableDynamicToolbarProps<TData>
...
export function DataTableDynamicToolbar<TData>({
	...
	const [visibleFilters, setVisibleFilters] = React.useState(
		new Set(table.getState().columnFilters.map(cf => cf.id)),
	)
	...
	return (
		<div
			...
			<div className='flex flex-1 flex-wrap items-center gap-2'>
				{table.options.enableGlobalFilter && <GlobalFilter table={table} />}
				...
				{visibleColums.map(column => (
					<div key={column.id} className='relative group'>
						<div
							...
							<Icons.cross className='size-3 stroke-3' />
						</div>
						<DataTableToolbarFilter key={column.id} column={column} />
					</div>
				))}
				{visibleFilters.size < columns.length && (
					<AddFilterButton onAdd={addFilter} columns={unfilteredColumns} />
				)}
				{isFiltered && (
					<Button
			...
			<div className='flex items-center gap-2'>
				{children}
				{showExportTable && <DataTableExportData table={table} />}
...
function GlobalFilter<TData>({
	table,
}: DataTableToolbarGlobalFilterProps<TData>) {
...
function AddFilterButton<TData>({
	onAdd,
	columns,
}: DataTableDynamicToolbarAddFilterProps<TData>) {
	const t = useTranslations('data-table')
	const [open, setOpen] = React.useState(false)
	return (
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<Button variant='outline' size='sm' className='border-dashed text-sm'>
